...
then
    ...
    eos ls -l /store/logs/prod/$year/$month/WMAgent/$wf
    logs=`eos ls -l /store/logs/prod/$year/$month/WMAgent/$wf | grep -i logCollect | head -1 | awk '{print $9}' `
else
    logs=$4
fi
echo $logs
...
eos cp /eos/cms/store/logs/prod/$year/$month/WMAgent/$wf/$logs ./public/ &
#xrdcopy "root://eoscms.cern.ch//cms/store/logs/prod/$year/$month/WMAgent/$wf/$logs?svcClass=archive" ./public/ &
